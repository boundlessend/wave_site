...
// 5 полос по ~6.67 единиц: 2-3-4-3-2, вся мишень ~33 единицы (треть шкалы)
// ponytail: значения подобраны на глаз, калибруются здесь же если игра окажется лёгкой/сложной
export const ZONE = {
  four: 3.33, // |d| <= 3.33  → 4 очка (центр)
  three: 10, // |d| <= 10   → 3 очка
  two: 16.67, // |d| <= 16.67 → 2 очка
} as const
